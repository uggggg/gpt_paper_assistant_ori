...
def get_papers_from_arxiv_api(area: str, timestamp) -> List[Paper]:
    # Use the Arxiv API to fetch papers based on a timestamp
    end_date = timestamp
    start_date = timestamp - timedelta(days=4)
    search = arxiv.Search(
        query="("
...
def get_papers_from_arxiv(area: str, config: Optional[dict]) -> List[Paper]:
    # Fetch papers directly from arxiv API without using RSS
    updated = datetime.utcnow() - timedelta(days=1)
    updated_string = updated.strftime("%a, %d %b %Y %H:%M:%S GMT")
    paper_list = get_papers_from_arxiv_api(area, updated)
    return paper_list
...
if __name__ == "__main__":
    config = configparser.ConfigParser()
    config.read("configs/config.ini")
    paper_list = get_papers_from_arxiv("cs.CL", config)
    print([paper.arxiv_id for paper in paper_list])
    print("Success")
